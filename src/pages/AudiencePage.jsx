...
// Avatar colour derived from index so it is always consistent
...
const INIT_REC = { first_name: '', last_name: '', phone: '', status: 'subscribed', source: 'manual' }
...
function AddRecipientModal({ onClose, onAdd, submitting }) {
  const [form, setForm] = useState(INIT_REC)
  const [error, setError] = useState('')
...
const AudiencePage = () => {
  ...
  const fetchContacts = async (segmentId) => {
    ...
    setContactsError('')
    try {
      const data = await getContacts(segmentId)
  ...
  const handleAddRecipient = async (form) => {
    setSubmitting(true)
    try {
      const created = await createContact({
        contact_list: selectedSegment.id,
      ...
      setSegments(prev => prev.map(s =>
        s.id === selectedSegment.id ? { ...s, contact_lenght: (s.contact_lenght ?? 0) + 1 } : s
      ))
      setSelectedSegment(prev => ({ ...prev, contact_lenght: (prev.contact_lenght ?? 0) + 1 }))
    } catch {
  ...
  const handleDeleteContact = async (contactId) => {
    try {
      ...
      setSegments(prev => prev.map(s =>
        s.id === selectedSegment.id ? { ...s, contact_lenght: Math.max(0, (s.contact_lenght ?? 1) - 1) } : s
      ))
      setSelectedSegment(prev => ({ ...prev, contact_lenght: Math.max(0, (prev.contact_lenght ?? 1) - 1) }))
    } catch {
      setContactsError('Failed to delete contact.')
    }
  }
              ...
              <div className="w-full max-w-6xl 2xl:max-w-5xl mx-auto">
                ...
                {contactsError && (
                  <div className="mb-4 flex items-center justify-between gap-3 bg-red-500/10 border border-red-500/40 text-red-400 rounded-xl px-4 py-3 text-sm">
                    ...
                    <button onClick={() => fetchContacts(selectedSegment.id)} className="text-xs underline hover:no-underline shrink-0">Retry</button>
                  </div>
                )}
                ...
                <div className="bg-[#1f2937]/60 border border-[#3e6ff4]/20 rounded-xl overflow-hidden">
                  {contactsLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                      ...
                      <p className="text-[#CAC4CF] text-sm mb-4">Add your first contact to this segment</p>
                      <button
                        onClick={() => setShowAddRecipientModal(true)}
                ...
                {contacts.length > 0 && (
                  <p className="text-xs text-[#CAC4CF]/50 mt-3">
                    {contacts.length} contact{contacts.length !== 1 ? 's' : ''} in this segment
                  </p>
                )}
              </div>
            </main>
