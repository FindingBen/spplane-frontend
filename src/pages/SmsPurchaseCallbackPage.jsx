            ...
            <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
              <section className="rounded-[28px] border border-emerald-500/25 bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.22),_transparent_38%),linear-gradient(135deg,rgba(15,23,42,0.95),rgba(17,24,39,0.92))] p-6 shadow-[0_24px_60px_rgba(2,6,23,0.35)] md:p-8">
                ...
                <div className="mt-5">
                  ...
                  <h1 className="mt-3 text-3xl font-bold text-white md:text-4xl">Your SMS package purchase was approved.</h1>
                  <p className="mt-3 max-w-2xl text-sm leading-6 text-[#d1d5db] md:text-[15px]">
              ...
              <section className="grid grid-cols-1 gap-5 lg:grid-cols-2">
                <article className="rounded-[24px] border border-[#3e6ff4]/20 bg-[#0f172a]/80 p-5">
                  ...
                  {loading ? (
                    <p className="mt-3 text-sm text-[#dbeafe]">Refreshing wallet balance...</p>
                  ) : wallet ? (
                    <>
                      <p className="mt-4 text-3xl font-bold text-white">{numberFormatter.format(availableCredits)}</p>
                      <p className="mt-2 text-sm text-[#CAC4CF]">Available SMS credits</p>
                      <div className="mt-5 grid grid-cols-2 gap-3 text-sm text-[#d1d5db]">
                        <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                          <p className="text-[11px] uppercase tracking-[0.18em] text-[#93c5fd]">Balance</p>
                          <p className="mt-2 text-lg font-semibold text-white">{numberFormatter.format(balance)}</p>
                        </div>
                        <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                          <p className="text-[11px] uppercase tracking-[0.18em] text-[#93c5fd]">Reserved</p>
                          <p className="mt-2 text-lg font-semibold text-white">{numberFormatter.format(reserved)}</p>
                        </div>
                      </div>
                    </>
                  ) : (
                    <p className="mt-3 text-sm text-[#dbeafe]">Wallet details are not available yet. Use the credits pill in the top bar to retry after a moment.</p>
                  )}
                </article>
                ...
                <article className="rounded-[24px] border border-[#3e6ff4]/20 bg-[#0f172a]/80 p-5">
                  ...
                  {loading ? (
                    <p className="mt-3 text-sm text-[#dbeafe]">Refreshing billing state...</p>
                  ) : billingStatus ? (
                    <>
                    ...
                    <p className="mt-3 text-sm text-[#dbeafe]">Billing details are not available yet. You can still return to packages and refresh from there.</p>
                  )}
                </article>
              </section>

              {refreshError && (
                <section className="rounded-2xl border border-amber-500/25 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
                  {refreshError}
                </section>
              )}
              ...
              <section className="rounded-[24px] border border-[#3e6ff4]/20 bg-[#0f172a]/75 p-5">
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-[#CAC4CF]">
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                  <Link
                    to="/sms-plans"
                    className="inline-flex items-center justify-center rounded-xl bg-[#3e6ff4]/15 px-4 py-2.5 text-sm font-semibold text-[#dbeafe] transition-all hover:bg-[#3e6ff4]/25 hover:text-white"
                  >
                    Back to packages
                  </Link>
                  <Link
                    to="/sms"
                    className="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-semibold text-white transition-all hover:border-[#3e6ff4]/35 hover:bg-white/10"
                  >
                    Go to SMS dashboard
                  </Link>
                </div>
