...
export default function Register() {
  ...
  // Detect Shopify user by presence of "shop" query param (e.g. ?shop=mystore.myshopify.com)
  const shopParam = searchParams.get('shop')
  const isShopify = Boolean(shopParam)
  const userType = isShopify ? 'shopify' : 'normal'
        ...
        <div className="bg-[#111827] border-2 border-gray-800 rounded-2xl shadow-lg overflow-hidden">
          ...
          <header className="text-center px-5 py-6 bg-[#1B2233]">
            <h3 className="text-2xl font-semibold text-white mb-1">
              {isShopify ? 'Connect Your Shopify Store' : 'Create an Account'}
            </h3>
            <p className="text-gray-400 text-sm">
              {isShopify
                ? `Setting up account for ${shopParam}`
          ...
          <div className="px-6 py-6 bg-[#111827]">
            {isShopify && (
              <div className="text-blue-400 text-sm mb-4 bg-blue-500/10 border border-blue-500/30 rounded-lg p-3">
                Shopify store detected. Your account will be linked to{' '}
                <span className="font-semibold">{shopParam}</span>.
              </div>
            )}
