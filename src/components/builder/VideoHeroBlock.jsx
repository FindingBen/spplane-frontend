...
export const VideoHeroPreview = ({ props = {} }) => (
  <div className="w-full relative">
    {props.fallbackImage ? (
      <img src={props.fallbackImage} alt={props.title || 'Video'} className="w-full h-40 object-cover" />
    ) : (
      <div className="w-full h-40 bg-gray-900 flex items-center justify-center">
        <span className="text-xs text-gray-400">No preview image set</span>
      </div>
    )}
    {/* Play button */}
    ...
    <div className="absolute top-2 right-2 flex gap-1">
      {props.autoplay && <span className="bg-black/60 text-white text-[8px] px-1 py-0.5 rounded">AUTO</span>}
      {props.loop && <span className="bg-black/60 text-white text-[8px] px-1 py-0.5 rounded">LOOP</span>}
    </div>
  </div>
)
