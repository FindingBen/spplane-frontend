...
const textareaCls =
  'min-h-[140px] w-full resize-none rounded-lg border border-[#3e6ff4]/30 bg-[#111827] px-4 py-3 text-xs text-white outline-none transition-colors placeholder:text-[#CAC4CF]/50 focus:border-[#3e6ff4]'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-[#3e6ff4]/30 bg-gradient-to-br from-[#1f2937] to-[#1D1A22] shadow-2xl">
        <div className="flex items-center justify-between border-b border-[#3e6ff4]/20 px-6 py-4">
          <div>
            <h2 className="text-base font-bold text-white">Send SMS</h2>
            <p className="mt-1 text-xs text-[#CAC4CF]">{customer.first_name || customer.last_name ? `${[customer.first_name, customer.last_name].filter(Boolean).join(' ')} • ` : ''}{customer.phone}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          <div>
            <label className="mb-1.5 block text-xs font-medium text-[#CAC4CF]">Sender Name</label>
            <input
              type="text"
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between gap-3">
              <label className="block text-xs font-medium text-[#CAC4CF]">Text Message</label>
              <button
                type="button"
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <button
              ...
              onClick={onClose}
              disabled={sending}
              className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-xs font-medium text-[#CAC4CF] transition-colors hover:bg-white/10 hover:text-white disabled:cursor-not-allowed disabled:opacity-60"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={sending}
              className="inline-flex items-center gap-2 rounded-xl border border-[#3e6ff4]/40 bg-[#3e6ff4]/10 px-4 py-2 text-xs font-semibold text-[#60a5fa] transition-colors hover:bg-[#3e6ff4]/20 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {sending ? (
                <>
